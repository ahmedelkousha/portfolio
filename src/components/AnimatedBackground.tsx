...
export const AnimatedBackground = () => {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <div
        className="absolute -top-40 -right-40 w-96 h-96 rounded-full opacity-30 blur-[100px] hidden"
        style={{
          background: "linear-gradient(135deg, hsl(var(--cyan)), hsl(var(--electric-blue)))",
        }}
      />
      <div
        className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full opacity-20 blur-[100px] hidden"
        style={{
      ...
      <motion.span
        className="absolute bottom-1/4 right-[15%] text-2xl font-mono text-cyan/20 select-none"
        animate={{ y: [-10, 10], rotate: [-30, 30] }}
        transition={{ duration: 6, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" }}
      >
        {"==="}
      ...
      <motion.span
        className="absolute bottom-1/4 left-[15%] text-2xl font-mono text-cyan/15 select-none"
        animate={{ y: [-10, 10], rotate: [-30, 30] }}
        transition={{ duration: 6, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" }}
      >
        {"[ ]"}
